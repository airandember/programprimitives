...
const mockAchievements = [
  ...
  { id: "10", name: "Year of Code", description: "365-day streak", category: "consistency", icon: "🌟", xpReward: 2e3, rarity: "legendary", unlocked: false }
];
const mockProgress = {
...
function getExercisesForPrimitive(primitiveId) {
  return mockExercises.filter((e) => e.primitiveId === primitiveId);
}
function getPrimitiveById(id) {
  return mockPrimitives.find((p) => p.id === id);
}
function getMasteryForPrimitive(primitiveId, language = "javascript") {
  return mockMastery.find((m) => m.primitiveId === primitiveId && m.language === language);
}
export {
  mockProgress as a,
  mockMastery as b,
  mockAchievements as c,
  getExercisesForPrimitive as d,
  getMasteryForPrimitive as e,
  mockExercises as f,
  getPrimitiveById as g,
  mockPrimitives as h,
  mockUser as m
};
