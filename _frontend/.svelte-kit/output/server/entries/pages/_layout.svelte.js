...
const Layout = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-1sqfmb9_START -->${$$result.title = `<title>ProgramPrimitives - Master the Physics of Code</title>`, ""}<!-- HEAD_svelte-1sqfmb9_END -->`, ""} <div class="min-h-screen flex flex-col"> <header class="sticky top-0 z-50 bg-surface-950/80 backdrop-blur-lg border-b border-surface-800"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex items-center justify-between h-16"> <a href="/" class="flex items-center gap-2" data-svelte-h="svelte-5nlxxm"><div class="w-8 h-8 rounded-lg bg-gradient-to-br from-primary-500 to-accent-500 flex items-center justify-center"><span class="text-white font-bold text-lg">P</span></div> <span class="font-display font-semibold text-lg hidden sm:block">Program<span class="text-gradient">Primitives</span></span></a>  <nav class="hidden md:flex items-center gap-1">${each(navItems, (item) => {
  ...
  })}</nav>  <div class="flex items-center gap-4">${$isAuthenticated ? ` <div class="hidden sm:flex items-center gap-4 text-sm"><div class="flex items-center gap-1 text-orange-500">${validate_component(Flame, "Flame").$$render($$result, { size: 16 }, {}, {})} <span>${escape($currentStreak)}</span></div> <div class="flex items-center gap-1 text-accent-400">${validate_component(Zap, "Zap").$$render($$result, { size: 16 }, {}, {})} <span>${escape($totalXp)} XP</span></div> <div class="badge badge-primary">Lvl ${escape($currentLevel)}</div></div>  <div class="relative" data-svelte-h="svelte-kemo95"><button class="flex items-center gap-2 p-1 rounded-full hover:bg-surface-800"><div class="w-8 h-8 rounded-full bg-gradient-to-br from-primary-500 to-accent-500"></div></button></div>` : `<a href="/login" class="btn btn-ghost text-sm" data-svelte-h="svelte-ojxb3g">Log in</a> <a href="/register" class="btn btn-primary text-sm" data-svelte-h="svelte-p0zpb9">Get Started</a>`}  <button class="md:hidden p-2 rounded-lg hover:bg-surface-800">${`${validate_component(Menu, "Menu").$$render($$result, { size: 24 }, {}, {})}`}</button></div></div></div>  ${``}</header>  <main class="flex-1">${slots.default ? slots.default({}) : ``}</main>  <footer class="border-t border-surface-800 py-8 mt-auto" data-svelte-h="svelte-1wohsye"><div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8"><div class="flex flex-col sm:flex-row items-center justify-between gap-4"><div class="flex items-center gap-2 text-surface-500 text-sm"><span>© 2024 ProgramPrimitives</span> <span>·</span> <a href="/privacy" class="hover:text-surface-300">Privacy</a> <span>·</span> <a href="/terms" class="hover:text-surface-300">Terms</a></div> <div class="flex items-center gap-4 text-surface-500"><a href="https://github.com" class="hover:text-surface-300">GitHub</a> <a href="https://twitter.com" class="hover:text-surface-300">Twitter</a> <a href="https://discord.com" class="hover:text-surface-300">Discord</a></div></div></div></footer></div>`;
});
export {
  Layout as default
};
