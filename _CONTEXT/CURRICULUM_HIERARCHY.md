# ProgramPrimitives Curriculum Hierarchy

> **The Core Vision:** You don't become a master craftsman by buying better tools — you **earn** refined tools through mastery of the basics.

## Blueprints

ProgramPrimitives teaches **universal programming tools and strategies** that transcend any single language. We focus on the fundamental building blocks every developer uses to solve problems.

### The Craftsman's Journey

Just as a craftsman progresses from crude implements to precision instruments, developers refine their tools through mastery:

```
📐 BLUEPRINTS     Understanding the why, planning the approach
        ↓
🪨 STONE TOOLS    Raw fundamentals (variables, output, basic logic)
        ↓
🪵 WOOD TOOLS     Structured basics (loops, conditionals, functions)
        ↓
🔩 BRONZE TOOLS   Data organization (arrays, objects, strings)
        ↓
⚙️ IRON TOOLS     Abstractions (callbacks, scope, error handling)
        ↓
🔧 STEEL TOOLS    Higher-order patterns (map, filter, reduce)
        ↓
⚡ POWER TOOLS    Advanced concepts (async, recursion, generators)
        ↓
🎯 PRECISION      Competition-level mastery (algorithms, optimization)
```

### The Tool Refinement Metaphor

- **Blueprints** → Understanding *why* before *how* — the mental models
- **Raw Tools** → Foundational concepts that everything else is built upon
- **Refined Tools** → Abstractions that emerge from mastering raw tools
- **Mastery** → Knowing when to use the chisel vs the power saw

A developer doesn't just "learn loops" — they **forge** their loop tool through:
```
while (stone) → for (wood) → for...of (bronze) → forEach (iron) → map (steel) → generators (power)
```

Each upgrade is **earned** through demonstrated understanding of the previous level.

---

## Tool-First, Language-Second Philosophy

### The Two Dimensions of Mastery

```
                        LANGUAGES
                   JS    Python    Go    Rust    ...
              ┌─────────────────────────────────────┐
    for-loop  │  ⚙️      🔧       🪵      🪨        │  
              ├─────────────────────────────────────┤
T   while     │  🔧      ⚙️       🪨      —         │
O             ├─────────────────────────────────────┤
O   map       │  🔧      🔧       🔩      🪨        │
L             ├─────────────────────────────────────┤
S   filter    │  ⚙️      🔧       🔩      —         │
              ├─────────────────────────────────────┤
    reduce    │  🪵      🔩       🪨      —         │
              └─────────────────────────────────────┘
```

**Tool Mastery** = Conceptual understanding (language-agnostic)
**Language Proficiency** = Syntax fluency in a specific language

### Learning Flow

```
1. UNDERSTAND THE TOOL (Concept)
   "What is a for loop? Why does it exist? When do I use it?"
   └── Language-agnostic explanation with pseudocode
   
2. LEARN THE SYNTAX (Per Language)
   "How do I write a for loop in JavaScript?"
   └── Language-specific syntax, idioms, gotchas
   
3. PRACTICE (Per Tool × Language)
   "Complete exercises in your chosen language"
   └── Exercises available in multiple languages
   
4. TRANSFER (Cross-Language)
   "Now try it in Python"
   └── Leverage conceptual knowledge, learn new syntax
```

### Why This Matters

| Benefit | Description |
|---------|-------------|
| **Transferable Skills** | Understand loops ONCE, apply ANYWHERE |
| **Language Flexibility** | Learn new languages faster |
| **Clear Progress** | See exactly where you're strong/weak |
| **Future-Proof** | New languages slot in naturally |
| **Polyglot Support** | Track multi-language proficiency |

---

## Language Support Architecture

### Supported Languages (Current)

| Language | Tier | Status | Notes |
|----------|------|--------|-------|
| JavaScript | Primary | ✅ Full | Most examples, default |
| Python | Primary | ✅ Full | Second priority |
| Go | Secondary | ✅ Partial | Growing support |
| TypeScript | Secondary | 🔲 Planned | Extends JS |
| Rust | Tertiary | 🔲 Future | Community interest |
| Java | Tertiary | 🔲 Future | Enterprise demand |
| C# | Tertiary | 🔲 Future | Game dev, enterprise |

### Adding a New Language

To add a new language, create:

```
1. Language definition in constants
   - id: 'rust'
   - name: 'Rust'
   - icon: '🦀'
   - fileExtension: '.rs'
   - syntaxHighlight: 'rust'

2. Syntax examples for each tool
   - For each primitive, add syntax_template + full_example
   
3. Starter code for exercises
   - Each exercise needs starter_code + solution_code per language
   
4. Sandbox support (if code execution)
   - Runtime configuration
   - Security constraints
```

### Language-Specific Considerations

Each language may have:
- **Unique idioms** — Python list comprehensions, Go channels
- **Missing concepts** — Go has no while loop, only for
- **Different terminology** — dict vs object vs map
- **Syntax variations** — Arrow functions, lambdas, closures

These are captured in:
```
primitive_syntax.variations  — Language-specific alternatives
primitive_syntax.explanation — Language-specific context
```

---

## Scalable Data Model

### Core Entities

```
TOOLS (primitives)
  │
  ├── LESSONS (per tool)
  │     └── Concept explanations, learning objectives
  │
  ├── SYNTAX (per tool × language)
  │     └── Language-specific examples
  │
  └── EXERCISES (per tool, available in multiple languages)
        └── Exercise definitions with per-language starter/solution
```

### Progress Tracking (Three Levels)

```typescript
// 1. TOOL MASTERY (Conceptual - Language Agnostic)
interface ToolMastery {
  toolId: string;
  conceptLevel: 'stone' | 'wood' | 'bronze' | 'iron' | 'steel' | 'mastered';
  lessonsCompleted: number;
  lessonsTotal: number;
}

// 2. LANGUAGE PROFICIENCY (Per Tool × Language)
interface ToolLanguageProficiency {
  toolId: string;
  language: string;
  syntaxLevel: 'stone' | 'wood' | 'bronze' | 'iron' | 'steel' | 'mastered';
  exercisesCompleted: number;
  exercisesTotal: number;
  averageScore: number;
  lastPracticed: Date;
}

// 3. OVERALL LANGUAGE SKILL (Aggregated)
interface LanguageSkill {
  language: string;
  toolsMastered: number;
  toolsTotal: number;
  overallLevel: number;  // Calculated from tool proficiencies
}
```

### Database Schema (Updated)

```sql
-- Languages table (for scalability)
CREATE TABLE languages (
    id TEXT PRIMARY KEY,           -- 'javascript', 'python', 'go'
    name TEXT NOT NULL,            -- 'JavaScript', 'Python', 'Go'
    display_order INTEGER DEFAULT 0,
    icon TEXT,                     -- emoji or icon class
    file_extension TEXT,           -- '.js', '.py', '.go'
    syntax_highlight TEXT,         -- Monaco/Prism language key
    sandbox_supported INTEGER DEFAULT 0,
    is_active INTEGER DEFAULT 1,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);

-- Tool mastery (conceptual, language-agnostic)
CREATE TABLE user_tool_mastery (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    tool_id TEXT NOT NULL,
    concept_level TEXT DEFAULT 'stone',  -- Conceptual understanding
    lessons_completed INTEGER DEFAULT 0,
    lessons_total INTEGER DEFAULT 0,
    last_studied_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    UNIQUE(user_id, tool_id)
);

-- Tool × Language proficiency (syntax fluency)
CREATE TABLE user_tool_language_proficiency (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    tool_id TEXT NOT NULL,
    language_id TEXT NOT NULL,
    syntax_level TEXT DEFAULT 'stone',
    exercises_completed INTEGER DEFAULT 0,
    exercises_total INTEGER DEFAULT 0,
    total_attempts INTEGER DEFAULT 0,
    successful_attempts INTEGER DEFAULT 0,
    average_score REAL DEFAULT 0,
    best_score INTEGER DEFAULT 0,
    total_time_minutes INTEGER DEFAULT 0,
    last_practiced_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    FOREIGN KEY (language_id) REFERENCES languages(id),
    UNIQUE(user_id, tool_id, language_id)
);

-- Overall language skill (aggregated view, can be computed or cached)
CREATE TABLE user_language_skill (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    language_id TEXT NOT NULL,
    tools_mastered INTEGER DEFAULT 0,  -- Tools at iron+ in this language
    tools_practiced INTEGER DEFAULT 0, -- Tools with any progress
    overall_level INTEGER DEFAULT 1,
    total_exercises INTEGER DEFAULT 0,
    total_xp INTEGER DEFAULT 0,
    last_active_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (language_id) REFERENCES languages(id),
    UNIQUE(user_id, language_id)
);
```

---

## Scalability Principles

### 1. Tool Definitions are Language-Agnostic

```yaml
# A tool definition (conceptual)
tool:
  id: for-loop
  name: For Loop
  category: control-flow
  tier: 2
  description: "Execute code a specific number of times..."
  # NO language-specific content here
```

### 2. Syntax is Attached Separately

```yaml
# Syntax attached per language
syntax:
  - tool_id: for-loop
    language: javascript
    template: "for (let i = 0; i < n; i++) { }"
    example: "for (let i = 0; i < 5; i++) { console.log(i); }"
    
  - tool_id: for-loop
    language: python
    template: "for i in range(n):"
    example: "for i in range(5):\n    print(i)"
    
  - tool_id: for-loop
    language: rust
    template: "for i in 0..n { }"
    example: "for i in 0..5 { println!(\"{}\", i); }"
```

### 3. Exercises Support Multiple Languages

```yaml
exercise:
  id: sum-to-n
  tool_id: for-loop
  title: "Sum of Numbers"
  description: "Calculate sum from 1 to n"
  # Exercise is conceptually the same
  
  starter_code:
    javascript: "function sumToN(n) {\n  // Your code\n}"
    python: "def sum_to_n(n):\n    # Your code\n    pass"
    go: "func sumToN(n int) int {\n    // Your code\n}"
    
  solution_code:
    javascript: "function sumToN(n) {\n  let sum = 0;\n  for (let i = 1; i <= n; i++) sum += i;\n  return sum;\n}"
    python: "def sum_to_n(n):\n    total = 0\n    for i in range(1, n + 1):\n        total += i\n    return total"
    # etc.
```

### 4. Progress Rolls Up Naturally

```
User completes for-loop exercises in JavaScript
  → user_tool_language_proficiency (for-loop, javascript) updated
  → user_tool_mastery (for-loop) concept level may increase
  → user_language_skill (javascript) aggregated totals updated

User then tries for-loop in Python
  → New user_tool_language_proficiency (for-loop, python) created
  → Conceptual mastery already high (transfers!)
  → Just learning syntax (faster progression)
```

### 5. Adding New Languages is Additive

```sql
-- Add Rust support
INSERT INTO languages (id, name, icon, file_extension, syntax_highlight, sandbox_supported)
VALUES ('rust', 'Rust', '🦀', '.rs', 'rust', 0);

-- Add syntax for existing tools (can be gradual)
INSERT INTO primitive_syntax (primitive_id, language, syntax_template, ...)
VALUES ('for-loop', 'rust', 'for i in 0..n { }', ...);

-- Add starter code for existing exercises (can be gradual)
INSERT INTO exercise_starter_code (exercise_id, language, starter_code, solution_code)
VALUES ('sum-to-n', 'rust', 'fn sum_to_n(n: i32) -> i32 { }', '...');
```

### 6. Adding New Tools is Additive

```sql
-- Add a new tool
INSERT INTO primitives (id, name, category, tier, ...)
VALUES ('pattern-matching', 'Pattern Matching', 'control-flow', 6, ...);

-- Add lessons for the tool
INSERT INTO lessons (tool_id, slug, title, ...) VALUES 
('pattern-matching', 'what-is-pattern-matching', 'What is Pattern Matching?', ...),
('pattern-matching', 'match-syntax', 'Match Expression Syntax', ...);

-- Add syntax per language (some languages may not support it)
INSERT INTO primitive_syntax (primitive_id, language, ...) VALUES
('pattern-matching', 'rust', 'match value { ... }', ...),
('pattern-matching', 'python', 'match value: case ...', ...);
-- Note: JavaScript doesn't have native pattern matching (yet)
```

---

## Future Language Considerations

### Languages on the Horizon

| Language | Why | Timeline |
|----------|-----|----------|
| TypeScript | JS superset, enterprise adoption | Near-term |
| Rust | Systems, WebAssembly, safety focus | Medium-term |
| Kotlin | Android, modern JVM | Medium-term |
| Swift | iOS/macOS development | Medium-term |
| Zig | Low-level, growing interest | Long-term |
| Mojo | AI/ML focused Python superset | Long-term |

### Language Feature Matrix

Some tools only exist in certain languages:

| Tool | JS | Python | Go | Rust |
|------|----|----|----|----|
| for-loop | ✅ | ✅ | ✅ | ✅ |
| while-loop | ✅ | ✅ | ✅* | ✅ |
| list-comprehension | ❌ | ✅ | ❌ | ❌ |
| pattern-matching | ❌ | ✅ | ❌ | ✅ |
| channels | ❌ | ❌ | ✅ | ❌ |
| ownership | ❌ | ❌ | ❌ | ✅ |

*Go uses `for` for everything

### Handling Language-Specific Tools

```yaml
tool:
  id: list-comprehension
  name: List Comprehension
  category: iteration
  tier: 5
  language_specific: true
  available_languages: ['python']
  related_tools: ['map', 'filter']  # For languages without this
  description: "Concise way to create lists from iterables"
```

When a tool isn't available in a user's preferred language:
> "List comprehensions are a Python-specific feature. In JavaScript, you'd use `map()` and `filter()` for similar results. [Learn map →](/learn/map)"

---

## Tool Anatomy: Lessons Within Tools

Every tool has its own **internal lesson progression**. This enables:
- Granular progress tracking per aspect
- Direct linking to specific lessons in code reviews
- Identifying exact knowledge gaps
- Building understanding systematically

### Lesson Structure Template

```
TOOL: {tool-id}
├── Lesson 1: {concept} — Introduction, basic syntax
├── Lesson 2: {concept} — Core use case
├── Lesson 3: {concept} — Common patterns
├── Lesson 4: {concept} — Edge cases & pitfalls
├── Lesson 5: {concept} — Advanced usage
└── Lesson 6: {concept} — Best practices & refinement
```

### Example: For Loop Lessons

```
TOOL: for-loop
├── L1: syntax-basics        — The three parts: init, condition, update
├── L2: counting             — Iterating a fixed number of times
├── L3: array-iteration      — Looping through array indices
├── L4: nested-loops         — Loops within loops
├── L5: off-by-one           — Common boundary errors
├── L6: loop-control         — Using break and continue
├── L7: patterns             — Accumulator, counter, search patterns
└── L8: optimization         — When to use for vs alternatives
```

**URL Structure:** `/learn/for-loop/off-by-one`
**Code Review Link:** "See [For Loop: Off-By-One Errors](/learn/for-loop/off-by-one)"

### Example: Arrays Lessons

```
TOOL: arrays
├── L1: what-is-array        — Ordered collections concept
├── L2: creating-arrays      — Declaration and initialization
├── L3: accessing-elements   — Index-based access (arr[0])
├── L4: modifying-elements   — Changing values at indices
├── L5: array-length         — Using .length property
├── L6: index-bounds         — Avoiding out-of-bounds errors
├── L7: iterating-arrays     — Looping through arrays
├── L8: common-methods       — push, pop, shift, unshift
├── L9: slice-splice         — Extracting and modifying sections
└── L10: searching           — indexOf, includes, find
```

### Example: Functions Lessons

```
TOOL: functions
├── L1: what-is-function     — Reusable code blocks concept
├── L2: defining-functions   — Function declaration syntax
├── L3: calling-functions    — Invoking with parentheses
├── L4: parameters           — Accepting inputs
├── L5: arguments            — Passing values when calling
├── L6: return-values        — Sending data back
├── L7: scope-intro          — Variables inside vs outside
├── L8: pure-functions       — No side effects concept
├── L9: naming-conventions   — Verb-based, descriptive names
└── L10: single-responsibility — One function, one job
```

### Lesson Difficulty Within a Tool

Lessons within a tool follow their own micro-progression:

| Lesson Position | Difficulty Modifier | Focus |
|-----------------|---------------------|-------|
| L1-L2 | Base | Syntax, basic concept |
| L3-L4 | Base + 0.5 | Core patterns, common usage |
| L5-L6 | Base + 1.0 | Edge cases, pitfalls |
| L7-L8 | Base + 1.5 | Advanced patterns |
| L9-L10 | Base + 2.0 | Best practices, optimization |

### Progress Tracking Per Lesson

```typescript
interface ToolProgress {
  toolId: string;
  overallMastery: 'stone' | 'wood' | 'bronze' | 'iron' | 'steel' | 'mastered';
  lessons: {
    [lessonId: string]: {
      status: 'locked' | 'available' | 'in_progress' | 'completed';
      completedAt?: Date;
      exercisesCompleted: number;
      exercisesTotal: number;
      bestScore: number;
      attempts: number;
    }
  }
}
```

### Unlocking Logic

- **L1** always available when tool is unlocked
- **L2+** requires previous lesson completed OR 70%+ score on assessment
- **Tool mastery level** = function of lessons completed + quality scores

### Code Review Integration

When reviewing user code, link directly to relevant lessons:

```markdown
**Issue:** Off-by-one error in your loop condition.

You wrote: `for (let i = 0; i <= arr.length; i++)`
Should be: `for (let i = 0; i < arr.length; i++)`

📚 Review: [For Loop: Off-By-One Errors](/learn/for-loop/off-by-one)
```

---

## The Complete Hierarchy

### TIER 0: 📐 Blueprints (Pre-Programming)
*Understanding the workshop before picking up tools*

| Concept | Description | Time |
|---------|-------------|------|
| What is Code? | Instructions computers understand | 15 min |
| Sequential Execution | Code runs top to bottom | 15 min |
| Computational Thinking | Breaking problems into steps | 30 min |
| Pseudocode | Planning before coding | 20 min |

**No exercises — conceptual foundation only**

---

### TIER 1: 🪨 Stone Tools (Foundational)
*Raw, essential implements — the first tools any craftsman learns*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `output` | Output/Printing | - | Display results to the user |
| `comments` | Comments | - | Notes explaining code |
| `variables` | Variables | output | Named storage for values |
| `data-types` | Data Types | variables | Numbers, strings, booleans |
| `operators` | Operators | data-types | Math and comparison operations |

**Refinement Path:**
- `variables` → `constants` → `typed-variables` → `destructuring`

**Exercises:** Basic, focused on syntax familiarity (5-10 per tool)

---

### TIER 2: 🪵 Wood Tools (Control Flow)
*Shaped and structured — deciding what to do and when*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `conditionals` | Conditionals (if/else) | operators | Make decisions |
| `boolean-logic` | Boolean Logic | conditionals | AND, OR, NOT combinations |
| `switch` | Switch Statements | conditionals | Multiple discrete options |
| `ternary` | Ternary Operator | conditionals | Inline conditionals |
| `for-loop` | For Loop | variables, operators | Repeat N times |
| `while-loop` | While Loop | conditionals | Repeat until condition |
| `loop-control` | Break & Continue | for-loop, while-loop | Exit or skip iterations |

**Refinement Paths:**
- `conditionals` → `ternary` → `guard-clauses` → `pattern-matching`
- `for-loop` → `for-of` → `forEach` → `higher-order-iteration`

**Exercises:** Logic puzzles, counting problems, simple algorithms

---

### TIER 3: 🔩 Bronze Tools (Data Structures)
*Stronger, more durable — organizing your workspace*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `arrays` | Arrays/Lists | for-loop | Ordered collections |
| `array-access` | Array Access & Modification | arrays | Reading and changing elements |
| `array-methods` | Array Methods | array-access, functions | push, pop, slice, etc. |
| `strings` | String Manipulation | arrays | Text as character collections |
| `objects` | Objects/Dictionaries | variables | Key-value pairs |
| `nested-structures` | Nested Data | arrays, objects | Complex data organization |

**Refinement Paths:**
- `arrays` → `array-methods` → `spread-operator` → `destructuring`
- `objects` → `object-methods` → `optional-chaining` → `computed-properties`

**Exercises:** Data transformation, search and filter, inventory systems

---

### TIER 4: ⚙️ Iron Tools (Functions)
*Reliable, versatile — tools that amplify effort*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `functions` | Function Basics | variables | Define reusable code blocks |
| `parameters` | Parameters & Arguments | functions | Inputs to functions |
| `return-values` | Return Values | functions | Outputs from functions |
| `scope` | Scope | functions, variables | Variable visibility |
| `pure-functions` | Pure Functions | functions, scope | No side effects |
| `callbacks` | Callbacks | functions | Functions as arguments |
| `closures` | Closures | scope, callbacks | Functions remembering scope |

**Refinement Paths:**
- `functions` → `arrow-functions` → `higher-order-functions` → `currying`
- `callbacks` → `closures` → `factory-functions` → `module-pattern`

**Exercises:** Code organization, building utilities, event handling simulation

---

### TIER 5: 🔧 Steel Tools (Iteration Mastery)
*Refined, precise — repeatable operations with elegance*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `for-of` | For...of / For...in | for-loop, arrays | Iterate over values |
| `foreach` | forEach | arrays, callbacks | Functional iteration |
| `map` | Map | foreach | Transform every element |
| `filter` | Filter | foreach, conditionals | Select elements |
| `reduce` | Reduce | map, filter | Combine into single value |
| `chaining` | Method Chaining | map, filter | Compose operations |
| `find-some-every` | Find, Some, Every | filter | Search and validate |

**Refinement Paths:**
- `for-loop` → `for-of` → `forEach` → `map/filter` → `reduce`
- `reduce` → `complex-reduce` → `transducers`

**Exercises:** Data pipelines, statistics, search algorithms

---

### TIER 6: 🛡️ Protective Gear (Error Handling)
*Safety equipment — protecting against the unexpected*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `try-catch` | Try/Catch | functions | Handle runtime errors |
| `error-types` | Error Types | try-catch | Different error categories |
| `throwing` | Throwing Errors | try-catch | Creating errors intentionally |
| `validation` | Input Validation | conditionals, functions | Check before processing |
| `edge-cases` | Edge Case Handling | validation | Handle boundaries |
| `defensive-coding` | Defensive Programming | edge-cases | Assume nothing |

**Refinement Paths:**
- `try-catch` → `specific-catches` → `custom-errors` → `result-types`

**Exercises:** Robust function writing, API response handling, user input

---

### TIER 7: 🔬 Specialized Tools (Advanced Data Structures)
*Purpose-built instruments for specific jobs*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `set` | Sets | arrays | Unique value collections |
| `map-structure` | Map (data structure) | objects | Any-type key collections |
| `stack` | Stacks | arrays | LIFO operations |
| `queue` | Queues | arrays | FIFO operations |
| `linked-list` | Linked Lists | objects, classes | Node-based sequences |
| `tree-basics` | Trees | linked-list | Hierarchical structures |
| `graph-basics` | Graphs | tree-basics | Relationship networks |

**Key Insight:** Know WHAT → Know WHEN → Know WHY (Big O)

**Exercises:** Implement from scratch, then use built-ins, compare performance

---

### TIER 8: ⚡ Power Tools (Recursion & Algorithms)
*Electric, automated — building tools that build tools*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `recursion-basics` | Recursion Fundamentals | functions, conditionals | Self-referential solutions |
| `base-cases` | Base Cases | recursion-basics | Termination conditions |
| `recursive-patterns` | Recursive Patterns | base-cases | Common recursive solutions |
| `memoization` | Memoization | recursion, closures | Cache results |
| `big-o` | Big O Notation | all above | Efficiency analysis |
| `divide-conquer` | Divide & Conquer | recursion | Binary search, merge sort |
| `backtracking` | Backtracking | recursion | Try all paths |

**Refinement Paths:**
- `brute-force` → `recursion` → `memoization` → `dynamic-programming`

**Exercises:** Classic CS problems, optimization challenges

---

### TIER 9: 🏭 Industrial Machinery (Async & Concurrency)
*Assembly lines — parallel processing at scale*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `callbacks-async` | Async Callbacks | callbacks | Non-blocking operations |
| `promises` | Promises | callbacks-async | Chainable async |
| `async-await` | Async/Await | promises | Readable async code |
| `promise-combinators` | Promise.all/race | promises | Parallel operations |
| `event-loop` | Event Loop | async-await | Understanding timing |
| `error-handling-async` | Async Error Handling | try-catch, async-await | Handling async failures |

**Refinement Paths:**
- `callbacks` → `promises` → `async-await` → `reactive-streams`

**Exercises:** API calls, timers, concurrent operations

---

### TIER 10: 🎯 Precision Instruments (Advanced Patterns)
*CNC, laser cutters — custom solutions with surgical precision*

| Tool ID | Name | Prereqs | Description |
|---------|------|---------|-------------|
| `generators` | Generators | iterators, functions | Lazy sequences |
| `iterators` | Custom Iterators | objects, generators | Protocol-based iteration |
| `decorators` | Decorators | higher-order-functions | Behavior modification |
| `proxies` | Proxies | objects | Operation interception |
| `symbols` | Symbols | objects | Unique identifiers |
| `metaprogramming` | Metaprogramming | all above | Code that writes code |

**Exercises:** Build frameworks, implement patterns from scratch

---

### TIER 11: 🏆 Master Craftsman (Algorithm Mastery)
*Competition ready — speed, precision, creativity under pressure*

| Category | Techniques | Complexity |
|----------|------------|------------|
| Searching | Binary search, BFS, DFS, A* | Medium-Hard |
| Sorting | Quick, Merge, Heap, Radix | Medium |
| Dynamic Programming | Memoization, Tabulation | Hard |
| Graph Algorithms | Dijkstra, Topological Sort | Hard |
| String Algorithms | KMP, Tries, Edit Distance | Hard |

**Benchmark:** Advent of Code Days 1-15

---

### TIER 12: 👑 Grandmaster (Expert Problem Solving)
*Elite artisan — combining everything under constraint*

- Complex multi-step problems
- Novel problem decomposition
- Optimization under constraints
- Time/space tradeoffs
- Elegant, maintainable solutions

**Benchmark:** Advent of Code Days 16-25, LeetCode Hard

---

## Current Primitive Mapping

### ✅ Implemented (in seed data)

| Primitive | Current Tier | Target Tier | Status |
|-----------|--------------|-------------|--------|
| variables | 1 | TIER 1 | ✅ Correct |
| operators | 1 | TIER 1 | ✅ Correct |
| conditionals | 1 | TIER 2 | ⚠️ Should be Tier 2 |
| for-loop | 2 | TIER 2 | ✅ Correct |
| while-loop | 2 | TIER 2 | ✅ Correct |
| functions | 2 | TIER 4 | ⚠️ Should be Tier 4 |
| arrays | 2 | TIER 3 | ⚠️ Should be Tier 3 |
| objects | 2 | TIER 3 | ⚠️ Should be Tier 3 |

### 🔲 Missing (Priority Additions)

**Tier 0-1 (Foundations):**
- [ ] output
- [ ] comments
- [ ] data-types

**Tier 2 (Control Flow):**
- [ ] boolean-logic
- [ ] switch
- [ ] ternary
- [ ] loop-control (break/continue)

**Tier 3 (Data):**
- [ ] array-methods
- [ ] strings
- [ ] nested-structures

**Tier 4-5 (Functions & Iteration):**
- [ ] parameters (detailed)
- [ ] return-values (detailed)
- [ ] scope
- [ ] map
- [ ] filter
- [ ] reduce

---

## Difficulty Calculation

Difficulty is NOT arbitrary — it's calculated from **dependency depth**:

```
difficulty = max(prereq_difficulties) + 1
```

| Tier | Material | Base Difficulty | Example |
|------|----------|-----------------|---------|
| 0 | 📐 Blueprints | 0 | Pre-programming concepts |
| 1 | 🪨 Stone | 1 | Variables, output |
| 2 | 🪵 Wood | 2 | Loops, conditionals |
| 3 | 🔩 Bronze | 3 | Arrays, objects |
| 4 | ⚙️ Iron | 4 | Functions, scope |
| 5 | 🔧 Steel | 5 | map/filter/reduce |
| 6 | 🛡️ Protective | 6 | Error handling |
| 7 | 🔬 Specialized | 7 | Advanced data structures |
| 8 | ⚡ Power | 8 | Recursion, algorithms |
| 9 | 🏭 Industrial | 9 | Async/concurrency |
| 10 | 🎯 Precision | 10 | Advanced patterns |
| 11 | 🏆 Master | 10+ | Algorithm mastery |
| 12 | 👑 Grandmaster | 10+ | Elite problem solving |

---

## Tool Refinement System

### Visual Progression

Each tool has refinement stages with visual upgrades (matching the material metaphor):

```
🪨 STONE      → First exposure, syntax learning
🪵 WOOD       → Completed 3+ exercises, getting comfortable
🔩 BRONZE     → Quality code, no hints needed
⚙️ IRON       → Can apply in novel situations
🔧 STEEL      → Efficient, elegant solutions
⚡ MASTERED   → Understands edge cases, teaches others
```

### Quality Metrics for Refinement

1. **Exercises Completed** — Volume of practice
2. **First-Try Success Rate** — Understanding depth
3. **Hint Usage** — Independence level
4. **Code Quality Score** — Industry standards
   - Naming conventions
   - Proper indentation
   - No code smells
   - Efficient solutions
5. **Time to Solution** — Fluency

### Unlocking Refined Tools

Users don't see `map()` until they've demonstrated:
- ✅ `for-loop` at PROFICIENT level
- ✅ `arrays` at COMPETENT level
- ✅ `callbacks` at PRACTICED level

Then `map()` unlocks with explanation:
> "You've been writing this pattern manually. Here's the tool that does it for you."

---

## Badge Categories

### 🛠️ Tool Mastery Badges
Earned by refining specific tools to mastery level

### 🔥 Streak Badges
Consistency over time (7, 14, 30, 100, 365 days)

### 🏆 Milestone Badges
Volume achievements (1, 5, 25, 100, 500 exercises)

### 🎯 Skill Badges
Special accomplishments (no hints, speed runs, first-try perfects)

### 🌟 Tier Badges
Completing all tools in a tier

### 👑 Elite Badges
Competition-level achievements (AoC completion, etc.)

---

## Implementation Priorities

### Phase 1: Foundation (Current → +2 weeks)
1. Update existing primitives with correct tiers
2. Add Tier 0-1 missing primitives
3. Implement prerequisite enforcement

### Phase 2: Control & Data (+2-4 weeks)
1. Add Tier 2-3 primitives
2. Create 5+ exercises per primitive
3. Implement basic refinement tracking

### Phase 3: Functions & Iteration (+4-8 weeks)
1. Add Tier 4-5 primitives
2. Implement quality scoring
3. Add tool unlocking system

### Phase 4: Advanced (+8-16 weeks)
1. Add Tier 6-10 progressively
2. Competition track (Tier 11-12)
3. Full refinement visualization

---

## Content Guidelines

### Every Primitive Needs:

1. **Clear Definition** — What is this tool?
2. **Why It Matters** — When/why would you use it?
3. **Syntax per Language** — JS, Python, Go (minimum)
4. **Best Practices** — Industry standards
5. **Common Pitfalls** — What trips people up
6. **Prerequisites** — What must be understood first
7. **Refinement Path** — What does this tool lead to?
8. **5-10 Exercises** — Progressive difficulty within the tool

### Exercise Quality Standards:

- **Clear Instructions** — No ambiguity
- **Real-World Context** — Why would someone do this?
- **Test Cases** — Comprehensive, including edge cases
- **Progressive Hints** — Nudge, don't solve
- **Solution Explanation** — Not just code, but WHY

---

---

## Complete Lesson Maps

### TIER 1: 🪨 Stone Tools — Lesson Breakdowns

#### `output` (Output/Printing)
```
├── L1: why-output           — Seeing what your code does
├── L2: print-syntax         — console.log / print / fmt.Println
├── L3: printing-variables   — Displaying stored values
├── L4: string-concatenation — Combining text and variables
├── L5: template-literals    — Modern string interpolation
└── L6: debugging-output     — Using print for debugging
```

#### `comments` (Code Comments)
```
├── L1: why-comments         — Code documentation purpose
├── L2: single-line          — // and # syntax
├── L3: multi-line           — /* */ and """ """ syntax
├── L4: when-to-comment      — What deserves a comment
├── L5: self-documenting     — Code that explains itself
└── L6: comment-antipatterns — Comments that hurt readability
```

#### `variables` (Variables)
```
├── L1: what-is-variable     — Named storage concept
├── L2: declaration          — Creating variables (let, const, var)
├── L3: assignment           — Storing values
├── L4: reassignment         — Changing values
├── L5: naming-conventions   — camelCase, snake_case, CONSTANTS
├── L6: const-vs-let         — When to use each
├── L7: initialization       — Declaring with initial values
└── L8: scope-preview        — Where variables exist
```

#### `data-types` (Data Types)
```
├── L1: what-are-types       — Different kinds of data
├── L2: numbers              — Integers and floats
├── L3: strings              — Text data
├── L4: booleans             — True/false values
├── L5: type-checking        — typeof, type(), reflect
├── L6: type-coercion        — Automatic conversion pitfalls
├── L7: null-undefined       — Absence of value
└── L8: choosing-types       — Which type for which situation
```

#### `operators` (Operators)
```
├── L1: what-are-operators   — Symbols that transform values
├── L2: arithmetic           — + - * / % **
├── L3: comparison           — == === != < > <= >=
├── L4: logical              — && || !
├── L5: assignment-ops       — += -= *= /=
├── L6: precedence           — Order of operations
├── L7: equality-gotchas     — == vs === pitfalls
└── L8: short-circuit        — && and || evaluation tricks
```

---

### TIER 2: 🪵 Wood Tools — Lesson Breakdowns

#### `conditionals` (If/Else)
```
├── L1: what-is-conditional  — Making decisions in code
├── L2: if-statement         — Single condition checks
├── L3: if-else              — Two-path decisions
├── L4: else-if              — Multiple conditions
├── L5: nested-conditionals  — Conditions within conditions
├── L6: truthy-falsy         — What counts as true/false
├── L7: guard-clauses        — Early returns for cleaner code
└── L8: refactoring-ifs      — Simplifying complex conditions
```

#### `boolean-logic` (Boolean Logic)
```
├── L1: and-operator         — Both must be true
├── L2: or-operator          — Either can be true
├── L3: not-operator         — Inverting boolean values
├── L4: combining-operators  — Complex conditions
├── L5: de-morgans-law       — Logical equivalences
├── L6: truthiness-tables    — Visualizing logic
└── L7: readable-conditions  — Writing clear boolean expressions
```

#### `for-loop` (For Loop)
```
├── L1: why-loops            — Repeating without copy-paste
├── L2: for-syntax           — init; condition; update
├── L3: counting-up          — i = 0; i < n; i++
├── L4: counting-down        — Reverse iteration
├── L5: step-values          — i += 2, i += 10
├── L6: array-iteration      — Looping through indices
├── L7: nested-loops         — Loops within loops
├── L8: off-by-one           — <= vs < boundary errors
├── L9: break-continue       — Exiting and skipping
└── L10: loop-patterns       — Accumulator, search, transform
```

#### `while-loop` (While Loop)
```
├── L1: while-vs-for         — When iteration count is unknown
├── L2: while-syntax         — while (condition) { }
├── L3: infinite-loops       — Danger and intentional use
├── L4: loop-variables       — Updating condition variables
├── L5: do-while             — Execute at least once
├── L6: sentinel-values      — Loop until special value
└── L7: while-patterns       — Input validation, polling
```

#### `switch` (Switch Statements)
```
├── L1: when-switch          — Multiple discrete values
├── L2: switch-syntax        — case, break, default
├── L3: fall-through         — Intentional and accidental
├── L4: grouping-cases       — Multiple cases, same action
├── L5: switch-vs-if         — When to use each
└── L6: switch-expressions   — Modern switch patterns
```

#### `ternary` (Ternary Operator)
```
├── L1: ternary-syntax       — condition ? yes : no
├── L2: simple-assignments   — Choosing between two values
├── L3: nested-ternary       — Why to avoid (usually)
├── L4: ternary-vs-if        — Readability tradeoffs
└── L5: ternary-patterns     — Default values, toggles
```

---

### TIER 3: 🔩 Bronze Tools — Lesson Breakdowns

#### `arrays` (Arrays/Lists)
```
├── L1: what-is-array        — Ordered collections
├── L2: creating-arrays      — Literal and constructor
├── L3: accessing-elements   — arr[index]
├── L4: modifying-elements   — arr[index] = value
├── L5: array-length         — .length property
├── L6: index-bounds         — Out-of-bounds errors
├── L7: iterating            — Looping through arrays
├── L8: push-pop             — Adding/removing from end
├── L9: shift-unshift        — Adding/removing from start
├── L10: slice               — Extracting portions
├── L11: splice              — Modifying in place
├── L12: spread-operator     — [...arr] spreading
└── L13: destructuring       — const [a, b] = arr
```

#### `objects` (Objects/Dictionaries)
```
├── L1: what-is-object       — Key-value collections
├── L2: creating-objects     — Literal syntax { }
├── L3: accessing-properties — dot and bracket notation
├── L4: modifying-properties — Adding and changing
├── L5: deleting-properties  — Removing keys
├── L6: checking-existence   — in operator, hasOwnProperty
├── L7: iterating-objects    — for...in, Object.keys/values
├── L8: nested-objects       — Objects within objects
├── L9: object-methods       — Functions as properties
├── L10: spread-operator     — {...obj} spreading
├── L11: destructuring       — const { a, b } = obj
└── L12: optional-chaining   — obj?.prop?.nested
```

#### `strings` (String Manipulation)
```
├── L1: string-basics        — Text as data
├── L2: string-length        — .length property
├── L3: accessing-chars      — str[index]
├── L4: concatenation        — Joining strings
├── L5: template-literals    — ${variable} interpolation
├── L6: case-conversion      — toUpperCase, toLowerCase
├── L7: searching            — indexOf, includes
├── L8: extracting           — slice, substring
├── L9: splitting            — str.split(delimiter)
├── L10: joining             — arr.join(delimiter)
├── L11: trimming            — trim, trimStart, trimEnd
└── L12: replacing           — replace, replaceAll
```

---

### TIER 4: ⚙️ Iron Tools — Lesson Breakdowns

#### `functions` (Functions)
```
├── L1: why-functions        — Reusability and organization
├── L2: function-declaration — function name() { }
├── L3: function-expression  — const fn = function() { }
├── L4: arrow-functions      — const fn = () => { }
├── L5: calling-functions    — fn() invocation
├── L6: parameters           — Accepting inputs
├── L7: arguments            — Passing values
├── L8: default-parameters   — Default values for params
├── L9: rest-parameters      — ...args collecting
├── L10: return-values       — Sending data back
├── L11: early-return        — Returning early for clarity
├── L12: void-functions      — Functions with no return
├── L13: naming-conventions  — Verb-based names
└── L14: documentation       — JSDoc, docstrings
```

#### `scope` (Variable Scope)
```
├── L1: what-is-scope        — Where variables exist
├── L2: global-scope         — Accessible everywhere
├── L3: function-scope       — Inside function only
├── L4: block-scope          — let/const in { }
├── L5: scope-chain          — Looking up the chain
├── L6: shadowing            — Same name, different scope
├── L7: var-vs-let           — Scope differences
└── L8: avoiding-globals     — Why globals are risky
```

#### `callbacks` (Callback Functions)
```
├── L1: functions-as-values  — Passing functions around
├── L2: callback-concept     — "Call me back when done"
├── L3: synchronous-callbacks — Array methods, event handlers
├── L4: callback-parameters  — Understanding callback args
├── L5: anonymous-callbacks  — Inline function definitions
├── L6: arrow-callbacks      — () => syntax in callbacks
├── L7: callback-patterns    — Error-first, completion
└── L8: callback-hell        — Why nesting gets messy
```

#### `closures` (Closures)
```
├── L1: closure-concept      — Functions remember their scope
├── L2: lexical-scope        — Where function was defined
├── L3: closure-examples     — Counter, private variables
├── L4: factory-functions    — Functions that make functions
├── L5: data-privacy         — Hiding implementation details
├── L6: closure-gotchas      — Loop variable capture
└── L7: practical-closures   — Event handlers, memoization
```

---

### TIER 5: 🔧 Steel Tools — Lesson Breakdowns

#### `map` (Array Map)
```
├── L1: map-concept          — Transform every element
├── L2: map-syntax           — arr.map(fn)
├── L3: callback-arguments   — element, index, array
├── L4: returning-values     — What to return
├── L5: chaining-map         — map().map()
├── L6: map-vs-foreach       — When to use each
├── L7: object-mapping       — Transforming object arrays
└── L8: map-patterns         — Extract, convert, compute
```

#### `filter` (Array Filter)
```
├── L1: filter-concept       — Select matching elements
├── L2: filter-syntax        — arr.filter(fn)
├── L3: predicate-functions  — Returning true/false
├── L4: filter-conditions    — Complex filtering
├── L5: chaining-filter      — filter().map()
├── L6: filter-vs-find       — All matches vs first match
└── L7: filter-patterns      — Search, validate, exclude
```

#### `reduce` (Array Reduce)
```
├── L1: reduce-concept       — Combine into single value
├── L2: reduce-syntax        — arr.reduce(fn, initial)
├── L3: accumulator          — The running result
├── L4: sum-example          — Classic reduce use
├── L5: object-building      — Reducing to objects
├── L6: flatten              — Reducing nested arrays
├── L7: reduce-vs-loop       — When reduce shines
└── L8: reduce-patterns      — Group, count, transform
```

---

## Complete Database Schema (Scalable)

All tables needed to support Tool × Language × Lesson architecture:

```sql
-- ============================================
-- LANGUAGES (Scalable language support)
-- ============================================

CREATE TABLE languages (
    id TEXT PRIMARY KEY,              -- 'javascript', 'python', 'go', 'rust'
    name TEXT NOT NULL,               -- 'JavaScript', 'Python', 'Go', 'Rust'
    display_order INTEGER DEFAULT 0,  -- Sort order in UI
    icon TEXT,                        -- Emoji or icon class
    file_extension TEXT,              -- '.js', '.py', '.go', '.rs'
    syntax_highlight TEXT,            -- Monaco/Prism language key
    sandbox_supported INTEGER DEFAULT 0,
    is_primary INTEGER DEFAULT 0,     -- Primary languages shown first
    is_active INTEGER DEFAULT 1,      -- Can be disabled without deletion
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL
);

-- Seed primary languages
INSERT INTO languages (id, name, display_order, icon, file_extension, syntax_highlight, sandbox_supported, is_primary, is_active, created_at, updated_at)
VALUES 
('javascript', 'JavaScript', 1, '🟨', '.js', 'javascript', 1, 1, 1, datetime('now'), datetime('now')),
('python', 'Python', 2, '🐍', '.py', 'python', 0, 1, 1, datetime('now'), datetime('now')),
('go', 'Go', 3, '🐹', '.go', 'go', 0, 0, 1, datetime('now'), datetime('now')),
('typescript', 'TypeScript', 4, '🔷', '.ts', 'typescript', 0, 0, 0, datetime('now'), datetime('now')),
('rust', 'Rust', 5, '🦀', '.rs', 'rust', 0, 0, 0, datetime('now'), datetime('now'));

-- ============================================
-- LESSONS (Per Tool)
-- ============================================

CREATE TABLE lessons (
    id TEXT PRIMARY KEY,
    tool_id TEXT NOT NULL,
    slug TEXT NOT NULL,               -- URL-safe: 'off-by-one', 'for-syntax'
    title TEXT NOT NULL,              -- 'Off-By-One Errors'
    description TEXT NOT NULL,        -- Brief description for listings
    sequence_order INTEGER NOT NULL,  -- L1, L2, L3...
    difficulty_modifier REAL DEFAULT 0, -- Added to tool base difficulty
    content_markdown TEXT,            -- Lesson content (conceptual)
    estimated_minutes INTEGER DEFAULT 10,
    is_premium INTEGER DEFAULT 0,
    is_published INTEGER DEFAULT 1,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    UNIQUE(tool_id, slug)
);

CREATE INDEX idx_lessons_tool ON lessons(tool_id);
CREATE INDEX idx_lessons_published ON lessons(is_published);

-- ============================================
-- USER PROGRESS: Tool Mastery (Conceptual)
-- ============================================

CREATE TABLE user_tool_mastery (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    tool_id TEXT NOT NULL,
    concept_level TEXT DEFAULT 'stone',  -- stone, wood, bronze, iron, steel, mastered
    lessons_completed INTEGER DEFAULT 0,
    lessons_total INTEGER DEFAULT 0,
    last_studied_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    UNIQUE(user_id, tool_id)
);

CREATE INDEX idx_tool_mastery_user ON user_tool_mastery(user_id);

-- ============================================
-- USER PROGRESS: Tool × Language Proficiency
-- ============================================

CREATE TABLE user_tool_language_proficiency (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    tool_id TEXT NOT NULL,
    language_id TEXT NOT NULL,
    syntax_level TEXT DEFAULT 'stone',   -- Proficiency in this language
    exercises_completed INTEGER DEFAULT 0,
    exercises_total INTEGER DEFAULT 0,
    total_attempts INTEGER DEFAULT 0,
    successful_attempts INTEGER DEFAULT 0,
    average_score REAL DEFAULT 0,
    best_score INTEGER DEFAULT 0,
    total_time_minutes INTEGER DEFAULT 0,
    last_practiced_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    FOREIGN KEY (language_id) REFERENCES languages(id),
    UNIQUE(user_id, tool_id, language_id)
);

CREATE INDEX idx_tool_lang_prof_user ON user_tool_language_proficiency(user_id);
CREATE INDEX idx_tool_lang_prof_tool ON user_tool_language_proficiency(tool_id);
CREATE INDEX idx_tool_lang_prof_lang ON user_tool_language_proficiency(language_id);

-- ============================================
-- USER PROGRESS: Lesson Progress
-- ============================================

CREATE TABLE user_lesson_progress (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    lesson_id TEXT NOT NULL,
    status TEXT DEFAULT 'locked',      -- locked, available, in_progress, completed
    started_at TEXT,
    completed_at TEXT,
    exercises_completed INTEGER DEFAULT 0,
    exercises_total INTEGER DEFAULT 0,
    best_score INTEGER,
    attempts INTEGER DEFAULT 0,
    time_spent_minutes INTEGER DEFAULT 0,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (lesson_id) REFERENCES lessons(id),
    UNIQUE(user_id, lesson_id)
);

CREATE INDEX idx_lesson_progress_user ON user_lesson_progress(user_id);

-- ============================================
-- USER PROGRESS: Overall Language Skill
-- ============================================

CREATE TABLE user_language_skill (
    id TEXT PRIMARY KEY,
    user_id TEXT NOT NULL,
    language_id TEXT NOT NULL,
    tools_mastered INTEGER DEFAULT 0,   -- Tools at iron+ level
    tools_practiced INTEGER DEFAULT 0,  -- Tools with any progress
    overall_level INTEGER DEFAULT 1,    -- Aggregated level
    total_exercises INTEGER DEFAULT 0,
    total_xp INTEGER DEFAULT 0,
    last_active_at TEXT,
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (language_id) REFERENCES languages(id),
    UNIQUE(user_id, language_id)
);

CREATE INDEX idx_lang_skill_user ON user_language_skill(user_id);

-- ============================================
-- LANGUAGE-SPECIFIC TOOL AVAILABILITY
-- (For tools that only exist in some languages)
-- ============================================

CREATE TABLE tool_language_support (
    id TEXT PRIMARY KEY,
    tool_id TEXT NOT NULL,
    language_id TEXT NOT NULL,
    is_supported INTEGER DEFAULT 1,      -- 1 = supported, 0 = not available
    alternative_tool_id TEXT,            -- If not supported, suggest this instead
    notes TEXT,                          -- "Use map() instead in JavaScript"
    created_at TEXT NOT NULL,
    FOREIGN KEY (tool_id) REFERENCES primitives(id),
    FOREIGN KEY (language_id) REFERENCES languages(id),
    FOREIGN KEY (alternative_tool_id) REFERENCES primitives(id),
    UNIQUE(tool_id, language_id)
);
```

---

## URL Structure

```
/learn                           → Tool catalog
/learn/{tool}                    → Tool overview + lesson list
/learn/{tool}/{lesson}           → Specific lesson content
/learn/{tool}/{lesson}/exercises → Exercises for this lesson

Examples:
/learn/for-loop                  → For Loop overview
/learn/for-loop/off-by-one       → Off-by-one errors lesson
/learn/for-loop/off-by-one/exercises → Practice exercises
```

---

## Maintenance

This document is the **source of truth** for curriculum structure. Any changes to:
- Primitive organization
- Difficulty ratings
- Prerequisite chains
- Tier assignments

Must be reflected here first, then implemented in code/database.

**Last Updated:** January 6, 2026
**Version:** 1.0
